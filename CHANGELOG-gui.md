...
- New Project Template System `#2105` `#2114` `#2125` `#2129`
  ...
  - Added "Open Custom Template Directory" bbutton for Custom templates
...
- Template ID handling in the Project Template system is now fully automatic: IDs are generated in the form `com.anatawa12.vrc-get.user.<uuid>`, are not shown or editable in the UI, and are referenced only internally. This prevents user error and simplifies the template creation process.
...
- Correctly save custom template Unity version ranges that include a dash (e.g., `2022.3.6-f1`) instead of normalizing them prematurely. `#9879596`
- Prevented duplicate entries from appearing in the Unity version dropdown in the template editor. `#9879596`
- Editing a custom template no longer fails with "Template with such id not found". Newly created templates now receive a stable ID on first save, and that ID is preserved on subsequent edits.
