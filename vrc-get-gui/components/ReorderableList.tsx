...
type NonFunction = string | number | boolean | null | undefined | symbol | bigint | object;
...
export function useReorderableList<T extends NonFunction>(
	...
	const setList = useCallback((value: SetStateAction<T[]>) => {
		if (typeof value === "function") {
			setBackedList((oldValue) => {
				let newValue = value(oldValue.map(({value}) => value)).map((value) => makeValue(value));
				if (newValue.length === 0 && !allowEmpty) newValue = [makeValue(defaultValue)];
				return newValue;
			});
		} else {
	...
	const add = useCallback((value: T, options?: AddOptions) => {
		if (options == null) {
			...
			setBackedList(old => {
				...
				if (idx === -1) return old;
				return [...old.slice(0, idx + 1), makeValue(value), ...old.slice(idx + 1)];
			})
