import {CheckForUpdateResponse, utilInstallAndUpgrade} from "@/lib/bindings";
...
import {UpdateStatusResult} from "@tauri-apps/api/updater";
...
export function CheckForUpdateMessage(
	...
	useTauriListen<DownloadProgressEvent>('tauri://update-download-progress', e => {
		console.log("e.payload.chunk_length", e.payload.chunkLength, e.payload.contentLength);
			...
			return (
				<DialogOpen>
					...
					<DialogDescription>
						...
						<p>{response.update_description}</p>
					</DialogDescription>
					<DialogFooter className={"gap-2"}>
						<Button onClick={close}>{tc("check update:dialog:dismiss")}</Button>
						<Button onClick={startDownload}>{tc("check update:dialog:update")}</Button>
					</DialogFooter>
				</DialogOpen>
			);
		case "downloading":
