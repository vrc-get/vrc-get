...
export function ProjectsGridCard({
    ...
    const setSortingStateMutation = useMutation({
        ...
        onMutate: async ({ sorting }) => {
            await queryClient.cancelQueries({ queryKey: ["environmentGetProjectSorting"] });
    ...
    const currentKey = sortingQuery.data.replace(/Reversed$/, "") as SimpleSorting;
    ...
    const toggleOrder = () => {
        const newSorting: Sorting = isReversed ? currentKey : `${currentKey}Reversed`;
    ...
    const projectsShown = useMemo(() => {
        ...
        const sorters: Record<SimpleSorting, (a: TauriProject, b: TauriProject) => number> = {
        ...
        <div className="flex flex-col h-full w-full overflow-hidden">
            <Card className="flex items-center mb-4 flex-wrap">
                <div className="flex items-center gap-2 m-2 ml-4">
                    ...
                    <Select value={currentKey} onValueChange={(value) => handleChangeSortingKey(value as SimpleSorting)}>
                ...
                <Button variant="ghost" size="icon" onClick={toggleOrder}>
                    {isReversed ? <ArrowUp className="size-4" /> : <ArrowDown className="size-4" />}
                </Button>
            </Card>
            <ScrollArea
                type="auto"
