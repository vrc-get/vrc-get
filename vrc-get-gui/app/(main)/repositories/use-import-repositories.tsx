...
export function useImportRepositories(
	...
	const [importRepositoryPick, pickDialog] = useFilePickerFunction(environmentImportRepositoryPick);
	...
	const startImportingRepositories = useCallback(async function startImportingRepositories() {
		try {
	...
	const downloadRepositories = useCallback(async function downloadRepositories(repositories: TauriRepositoryDescriptor[]) {
		try {
			const totalCount = repositories.length;
			const [cancel, resultPromise] = callAsyncCommand(environmentImportDownloadRepositories, [repositories], (downloaded) => {
				setState({type: 'loadingRepositories', totalCount, downloaded, cancel});
			});
			setState({type: 'loadingRepositories', totalCount, downloaded: 0, cancel});
	...
	const addRepositories = useCallback(async function addRepositories(repositories: TauriRepositoryDescriptor[]) {
		try {
	...
	switch (state.type) {
		...
		case "confirmingRepositories":
			dialogBody = <ConfirmingRepositoryList
		...
		case "loadingRepositories":
			dialogBody = <LoadingRepositories cancel={cancel} downloaded={state.downloaded} totalCount={state.totalCount}/>;
			break;
		case "confirmingPackages":
			dialogBody = <ConfirmingPackages
	...
	const confirmDialog = dialogBody ?
		<DialogOpen>
			<DialogTitle>{tc("vpm repositories:dialog:import repositories")}</DialogTitle>{dialogBody}
		</DialogOpen> : null;
...
function shortRepositoryDescription(repo: TauriRepositoryDescriptor): React.ReactNode {
	if (Object.keys(repo.headers).length > 0) {
		return tc("vpm repositories:dialog:repository with headers", {repoUrl: repo.url});
	}
	return repo.url;
}
...
function ConfirmingRepositoryList(
	...
	const onContinue = useCallback(async function onContinue() {
	...
	return (
		<>
							...
							<ul className={"list-disc pl-6"}>
								{
									pickResult.unparsable_lines.map((line, idx) => (
										<li key={idx} className={"whitespace-pre"}>{line}</li>
									))
								}
							</ul>
						</>
			...
			<DialogFooter className={"gap-2"}>
				...
				<Button onClick={onContinue}>{tc("vpm repositories:dialog:button:continue importing repositories")}</Button>
			</DialogFooter>
		</>
	);
}
...
function LoadingRepositories(
	...
	return (
		<>
			<DialogDescription>
				<p>
					{tc("vpm repositories:dialog:downloading repositories...")}
				</p>
				<Progress value={downloaded} max={totalCount}/>
				<div className={"text-center"}>
					{tc("vpm repositories:dialog:downloaded n/m", {downloaded, totalCount})}
				</div>
			</DialogDescription>
			<DialogFooter>
				<Button onClick={cancel}>{tc("general:button:cancel")}</Button>
...
function ConfirmingPackages(
	...
	const add = useCallback(async function add() {
	...
	return (
		<>
			...
			<DialogDescription className={"font-normal"}>
				...
				<Accordion type="single" collapsible className="max-h-[50vh] overflow-y-auto w-full">
					{
						repositories.map(([repo, download]) => {
							...
							switch (download.type) {
								...
								case "Duplicated":
									error = true;
									content = tc("vpm repositories:dialog:download error:duplicated");
									break;
								case "DownloadError":
									error = true;
									content = tc("vpm repositories:dialog:download error:download error");
									break;
								case "Success":
									error = null;
									content = (
							...
							return (
								<AccordionItem value={repo.url} key={repo.url}>
									<AccordionTrigger
										className={`${destrucive} py-2 text-base`}>
			...
			<DialogFooter>
				<Button onClick={cancel}>{tc("general:button:cancel")}</Button>
				<Button onClick={add} className={"ml-2"}>{tc("vpm repositories:button:add repositories")}</Button>
			</DialogFooter>
		</>
	);
}
...
function AddingRepositories() {
	return (
		<>
			<DialogDescription>
				<p>
					{tc("vpm repositories:dialog:adding repositories...")}
				</p>
			</DialogDescription>
		</>
	);
