...
import type {PackageRowInfo, PackageLatestInfo} from "@/app/projects/manage/collect-package-row-info";
...
export const PackageListCard = memo(function PackageListCard(
  {
    ...
    projectPath: string;
    createChanges: (operation: RequestedOperation, create: Promise<TauriPendingProjectChanges>) => void;
  ...
  const hiddenUserRepositories = useMemo(() => new Set(repositoriesInfo?.hidden_user_repositories ?? []), [repositoriesInfo]);
  ...
  const onReinstallRequest = useCallback(() => createChanges({type: "reinstallAll"}, projectResolve(projectPath)), [createChanges, projectPath]);

  const onRemoveRequested = useCallback(async (pkg: PackageRowInfo) =>
    createChanges({type: "remove", displayName: pkg.displayName},
      projectRemovePackages(projectPath, [pkg.id])), [createChanges, projectPath]);
  ...
  const onInstallBulkRequested = useCallback(() => {
    try {
      ...
      createChanges({type: "bulkInstalled", hasUnityIncompatibleLatest},
  ...
  const onRemoveBulkRequested = useCallback(() => {
    createChanges({type: "bulkRemoved"}, projectRemovePackages(projectPath, bulkUpdatePackageIds.map(([id, _]) => id)));
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent className={"max-h-96 w-64"}>
          ...
          <UserLocalRepositoryMenuItem
            hideUserLocalPackages={repositoriesInfo?.hide_local_user_packages ?? false}
            refetch={onRefreshRepositories}
          />
          <hr className="my-3"/>
          {
...
function BulkUpdateCard(
  ...
  return (
    <Card className={"flex-shrink-0 p-2 flex flex-row gap-2 bg-secondary text-secondary-foreground flex-wrap"}>
      {canInstall && <ButtonDisabledIfLoading onClick={bulkInstallAll}>
        {tc("projects:manage:button:install selected")}
      </ButtonDisabledIfLoading>}
      {canUpgrade && <ButtonDisabledIfLoading onClick={bulkUpgradeAll} variant={"success"}>
        {tc("projects:manage:button:upgrade selected")}
      </ButtonDisabledIfLoading>}
      {canRemove && <ButtonDisabledIfLoading onClick={bulkRemoveAll} variant={"destructive"}>
...
function RepositoryMenuItem(
  ...
  return (
    <DropdownMenuItem className="p-0" onSelect={(e) => {
      e.preventDefault()
    }}>
  ...
  return (
    <>
      <td className={`${cellClass} w-1`}>
        <CheckboxDisabledIfLoading checked={bulkUpdateSelected}
      ...
      <td className={noGrowCellClass}>
        <PackageVersionSelector pkg={pkg} onInstallRequested={onInstallRequested}/>
      </td>
...
const PackageVersionSelector = memo(function PackageVersionSelector(
  ...
  const onChange = useCallback((version: string) => {
    ...
    if (!pkgVersion) return;
    onInstallRequested(pkgVersion);
  }, [onInstallRequested, pkg.installed, pkg.unityCompatible, pkg.unityIncompatible]);
  ...
  return (
    ...
    >
      ...
      <SelectContent>
        ...
        {isOpen && <PackageVersionList versionNames={versionNames} incompatibleNames={incompatibleNames}/>}
      </SelectContent>
    </Select>
  );
})
