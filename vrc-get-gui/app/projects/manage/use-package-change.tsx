...
interface PackageChangeDialog {
  createChanges: (operation: RequestedOperation, createPromise: Promise<TauriPendingProjectChanges>) => void;
...
export function usePackageChangeDialog(
  ...
  const createChanges = useCallback(async (operation: RequestedOperation, createPromise: Promise<TauriPendingProjectChanges>) => {
    try {
      setInstallStatus({status: "creatingChanges"});
      const changes = await createPromise;
      setInstallStatus({status: "promptingChanges", changes, requested: operation});
    } catch (e) {
      console.error(e);
      toastThrownError(e);
...
function comparePackageChangeByName([aName, _1]: [string, TauriPackageChange], [bName, _2]: [string, TauriPackageChange]): number {
