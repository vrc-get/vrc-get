import React, {ComponentProps, forwardRef, Fragment, useContext, useState} from "react";
...
export function ProjectRow(
  ...
  return <ProjectRowContext.Provider value={{removed, loading: Boolean(loading)}}>
    <tr className={`even:bg-secondary/30 ${(removed || loading) ? 'opacity-50' : ''}`}>
      ...
      <td className={`${cellClass} max-w-64 overflow-hidden`}>
        <Tooltip>
          <TooltipTriggerIfRemoved className={"text-left select-text cursor-auto w-full"}>
            <div className="flex flex-col">
              <Tooltip>
                <TooltipTriggerIfExists className={"text-left select-text cursor-auto w-full"}>
      ...
      <td className={noGrowCellClass}>
        <div className="flex flex-row gap-2 max-w-min">
          <ButtonDisabledIfRemoved
            onClick={() => openUnity(project.path, project.unity, project.unity_revision)}>{tc("projects:button:open unity")}</ButtonDisabledIfRemoved>
          <ManageOrMigrateButton project={project} refresh={refresh}/>
          <ButtonDisabledIfRemoved onClick={() => backupProjectModal.startBackup(project)}
                     variant={"success"}>{tc("projects:backup")}</ButtonDisabledIfRemoved>
          <DropdownMenu>
...
function ManageOrMigrateButton(
  ...
  switch (project.project_type) {
    ...
    case "Avatars":
    case "VpmStarter":
      return <ButtonDisabledIfRemoved
...
function MigrateButton(
  ...
  const [dialogStatus, setDialogStatus] = useState<MigrateState>({type: 'normal'});
      ...
      dialogContent = (
        <DialogOpen className={"whitespace-normal"}>
          ...
          <DialogFooter>
            ...
            <Button onClick={() => doMigrateVpm(true)}
                    variant={"destructive"}>{tc("projects:button:migrate in-place")}</Button>
          </DialogFooter>
        </DialogOpen>
      );
      break;
...
const ProjectRowContext = React.createContext<{ removed: boolean, loading: boolean }>({
  removed: false,
  loading: false,
});

const ButtonDisabledIfRemoved = forwardRef<HTMLButtonElement, React.ComponentProps<typeof Button>>(function RemovedButton(props, ref) {
  ...
  if (rowContext.removed) {
    ...
    return (
      <Button {...props} className={`disabled:pointer-events-auto ${props.className}`}
              disabled={props.disabled || rowContext.loading || rowContext.removed} ref={ref}/>
    );
  }
});

const TooltipTriggerIfRemoved = ({children, ...props}: ComponentProps<typeof TooltipTrigger>) => {
  const rowContext = useContext(ProjectRowContext);
  if (rowContext.removed) {
...
const TooltipTriggerIfExists = ({children, ...props}: ComponentProps<typeof TooltipTrigger>) => {
  const rowContext = useContext(ProjectRowContext);
  if (rowContext.removed) {
    return children;
  } else {
