...
impl DatabaseConnection {
    ...
    pub fn get_projects(&self) -> Result<Box<[Project]>> {
        unsafe {
            ...
            let result = vrc_get_litedb_database_connection_get_projects(self.ptr.get(), &mut slice).into_result();
            let boxed = slice.as_boxed_byte_slice_option();

            result?; // return if error
            
            return Ok(boxed.unwrap().into_vec().into_iter().map(|x| {
                Project::from_ffi(x)
            }).collect());
        }
    }
...
extern "C" {
    ...
    fn vrc_get_litedb_database_connection_get_projects(ptr: isize, out: &mut FFISlice<ProjectFFI>) -> LiteDbErrorFFI;
}
